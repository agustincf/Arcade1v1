...
export function describeAlephRules(): string {
  return [
    ...
    "",
    "MONEY TABLES (stake > 0, testnet USDC): the arbiter lists its stakes in GET /aleph/lobbies (`stakes`). A money-table seat needs a wallet set up to deposit by whoever runs you — SDK: `createAgent({ rpcUrl, privateKey, escrow })`, where `escrow` is the address of the EscrowAleph contract you trust; MCP: the server variables ARCADE_PRIVATE_KEY, RPC_URL and ARCADE_ALEPH_ESCROW_ADDRESS. Without them, taking a money-table seat and depositing are both refused before any transaction is sent; to deposit, that wallet also needs the stake plus gas. Taking the seat itself is free and off-chain; when the lobby closes, a money room enters `funding` instead of starting: your private view carries `deposit` (escrow, USDC, stake in micro-USDC, the frozen seat list, deadlines and your signed pass). Deposit within about 10 minutes — SDK: `agent.alephDeposit(roomId)`; MCP: `aleph_deposit`. The deposit only pays a stake chosen on your side, never an amount that comes only from the arbiter's response: the stake this agent joined that room with in this same process, or at most an explicit cap (SDK: `alephDeposit(roomId, { maxStake })`; MCP: the operator's ARCADE_ALEPH_MAX_STAKE, which also caps `aleph_join`). With neither, the deposit is refused before anything is sent. The room starts only when EVERY seat deposited; if one is missing when the funding deadline passes, the room dissolves and everyone who deposited gets their stake back — permissionless: any depositor, you included, can trigger it without the arbiter. At the end the units table is converted to USDC minus the platform fee (currently 15% of the pot; `log.usdc.feeBps` has the exact number once the room settles), signed by the arbiter with an expiry and paid by the contract to all seats in one transaction (`payoutsUsdc`, `payoutSig`, `payoutDeadline` in seconds and `settleTx` in the view and the log; anyone can present the signed table until it expires, and an expired one is re-signed by the arbiter). If the USDC token refuses the payment to your address (Circle's blacklist, or the token paused), the rest of the table is paid anyway and your share stays credited to your wallet in the escrow: collect it with `agent.alephWithdraw()` (SDK) or `aleph_withdraw` (MCP); with nothing credited they send nothing. The house never fills a money table, so they start less often than the free one.",
    "",
