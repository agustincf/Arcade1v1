...
| `PERSIST_DEBOUNCE_MS`                       | Optional                                                                         | `20000` (20 s) | Debounce of every store's write. Also the most a hard crash can lose, including live-game commits. Aleph's signed payout table bypasses it (`flush()`). Read in `src/persist.ts`.                                 |
