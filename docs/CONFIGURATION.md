...
| `ALEPH_PLAY_WINDOW_MS`     | Optional                                                       | `10800000` (3 h)  | Play window written into every seat's pass as the on-chain `playDeadline`, counted from when funding starts (`fundingDeadline + ALEPH_PLAY_WINDOW_MS`). After it plus the contract's 30-minute grace, anyone can call the contract's `refundExpired` to get their stake back. A room plays at most N+2 stages (~22 phases of `ALEPH_PHASE_MS`, 2 min by default), so 3 h is generous. Read in `src/aleph.ts`.                                                                                                                                                                         |
