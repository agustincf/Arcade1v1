...
| --------------------- | ---------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `ARBITER_PRIVATE_KEY` | **Required in production if either escrow is active** (see fail-fast guard below)                                | none                                                                                                                               | Private key of the arbiter wallet that signs match results (EIP-712) and pays gas for on-chain refunds/cancellations. Generate with `cast wallet new`. Read in `src/sign.ts`, `src/onchain.ts`, `src/demo-live.ts`.                                                                                                                    |
...
| `RPC_URL`             | **Required in production if either escrow is active**                                                            | none (`onchain.ts`/`onchain-e2e.ts` fall back to `http://localhost:8545`; `demo-live.ts` falls back to `https://sepolia.base.org`) | RPC endpoint used by the arbiter to cancel/refund matches on-chain (draws and expired matches) and by the gas monitor to check its own balance. Should be a dedicated provider (Alchemy/QuickNode), not a public endpoint, once real money is involved.                                                                                |
...
and every settle reverts (see `docs/REDEPLOY-escrow-aleph-v2.md`).
