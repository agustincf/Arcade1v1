...
2. Escuchar en modo `starting` (esto depende de que el **Health Check Path**
   de Render esté en `/health`: con solo el chequeo de puerto, Render pasaría
   el tráfico apenas se abre, sin esperar a este modo).
3. Según la posta:
   - **no existe, está soltada, o está vencida SIN registro** (huérfana: su dueña sacó la época y se cayó antes de escribirla): tomarla (`INCR`) → `Traspaso: none | released | stale`.
   - **está vencida CON registro** (una dueña que puede seguir viva: sus latidos no llegaron, por ejemplo tras una caída de Upstash): tocarle el timbre una vez antes de tomarla. Si contesta, esperar a que SUELTE su época — nunca tomarle la posta a una dueña que acaba de contestar que está viva — y recién ahí tomarla → `Traspaso: doorbell`. Si no contesta, tomarla directo → `Traspaso: stale`.
...
1. **Solo si vino por el timbre, validarlo:** el pedido lleva una firma —un HMAC con el token de Upstash sobre la época pedida y el momento— que se chequea ANTES de tocar Redis; sin ella, 403, sin gastar ni una lectura. Con la firma en regla, leer `arcade:lease:handover`: tiene que ser para mi época, de otra instancia y de menos de 60 s. Si no lo es, 409 y no pasa nada. La autoridad de fondo sigue en Redis, donde solo escriben nuestras instancias. Tiene un límite de un toque cada 2 s. El SIGTERM no se valida: lo manda Render.
...
| Deploy cancelado (la nueva tomó la posta y la soltó sin cargar) | La vieja ve la posta libre en otra época y sale; Render la reinicia y carga de nuevo. Unos 90 s de 503 más el reinicio (~1 min).                                                                                                                                                                                                                                                       |
| La nueva se cae después de cargar y antes de estar sana         | Durante un deploy, Render NO reinicia una instancia nueva que se cayó: el deploy queda fallido. La vieja (`released`) ve la posta en manos de una instancia muerta (la nueva, en su propia época); espera a que venza (≤ 3 min) y, como esa época NUNCA es la suya, espera otros 90 s libres y sale con 1 (3.5.1) para que Render la reinicie y recargue. En total, unos 5 min de 503. |
...
| Alguien toca el timbre desde afuera                             | Sin la firma HMAC válida, 403 SIN tocar Redis (ver 3.5, paso 1). Con firma pero sin un pedido válido en Redis, 409. Como mucho, una lectura chica por toque, con límite.                                                                                                                                                                                                               |
