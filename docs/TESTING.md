...
| `apps/server` (53)        | Core 1v1: `agents-routes`, `agents`, `anti-espionage`, `cancel-onchain`, `challenge-routes`, `challenge`, `cola-onchain`, `config-guard`, `deposito-onchain`, `failed-attempts`, `funnel-stats`, `gas-monitor`, `house-agents`, `paid-settle`, `profiles-routes`, `profiles`, `ratings-multi`, `rules-version`, `stats`, `submit-race`, `tick-budget`. Aleph: `aleph-funding-no-escrow`, `aleph-funding`, `aleph-game`, `aleph-house`, `aleph-lobby`, `aleph-money-durability`, `aleph-routes`, `aleph-rules-version`, `aleph-sdk-e2e`, `aleph-sign`. Live games: `live-deposit`, `live-durable`, `live-flappy`, `live-routes`, `live-runner`, `live-sdk-e2e`, `live-store`, `live-views`, `live-webhook-routes`, `live-webhook-runner`. Deploy handoff and persistence: `handover-sim`, `handover`, `jobs`, `lease`, `persist-guard`, `readiness`, `redis`, `version`. BYO webhooks: `webhook-agents`, `webhook-fetch`, `webhook-model`, `webhook-routes` (all `*.test.ts`) | `node:test`            |
...
- **`check-deploy.sh`** — Runs the real deploy script (`script/Deploy.s.sol`)
  ...
  ```
- **`check-integration.sh`** — Verifies that the two EIP-712 digests the
  arbiter signs (via `viem`, in `apps/server/src/digestcheck.ts`) are
  byte-identical to the digests the deployed contract computes: the result
  (`resultDigest`, with its expiry) and the seat (`seatDigest`, with the stake
  and deadlines). If they match, the arbiter's signatures are valid for the
  contract to accept.
  ```bash
  bash packages/contracts/check-integration.sh
  ```
- **`check-payment-e2e.sh`** — Full payment path on a local `anvil` chain:
  deploys `BlacklistUSDC` + `Escrow1v1`, has both players deposit with the
  terms the arbiter signed into their seats, and confirms the arbiter decides,
  signs and **settles by itself** (`apps/server/src/onchain-e2e.ts`): the
  winner gets the prize and the platform the commission without anyone else
  sending a transaction. It also covers the winner presenting the same
  signature while the arbiter's is in flight (the arbiter's reverts mined and
  it records `"external"`, no double pay), a blacklisted winner (the prize is
  credited and delivered later with `withdrawFor`), a draw, a player who never
  deposited, and two mined-but-reverted `cancelMatch` races, which must not
  count as refunds. The arbiter signs with anvil's public account #9, fixed in
  the script; it reads no `.env`.
  ```bash
...
- **`check-aleph-e2e.sh`** — Full Aleph money-table path on a local `anvil`
  chain with the real arbiter: deploys `BlacklistUSDC` + `EscrowAleph`, has 4
  ...
  refunded; a seat presents the table before the arbiter's `settle` (mined
  reverted, not taken as a payment); a seat is blacklisted before settlement,
  the other three are paid, its share stays credited, `alephWithdraw` fails
  while it is blacklisted and collects once it is not; and a deposit with
  another seat's `open` still pending.
  ```bash
