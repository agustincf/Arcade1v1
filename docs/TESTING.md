...
- `apps/web` beyond the five files above — no component, hook, or
  page-level tests, and no browser/E2E automation.
...
- **`check-aleph-e2e.sh`** — Full Aleph money-table path on a local `anvil`
  ...
  platform, escrow left at zero); then repeats with an incomplete funding that
  the arbiter cancels and confirms only the depositors are refunded.
  ```bash
