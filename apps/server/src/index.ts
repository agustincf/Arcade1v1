...
app.get("/", (_req, res) =>
  res.json({
    ...
    endpoints: {
      ...
      "POST /vault/join":
        "{ stake: 0, address, signature, ts } -> a seat in Aleph, the 4–8 agent room (sign matchmakeAuthMessage('vault', stake, address, ts)). Rules: @arcade1v1/game-sdk/vault (VAULT_RULES, rulesV)",
