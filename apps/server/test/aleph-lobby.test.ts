...
// Correr: node --import tsx --test apps/server/test/aleph-lobby.test.ts
...
process.env.ALEPH_MAX_ROOMS = "2";
process.env.ALEPH_MAX_SETTLED_KEPT = "2";
...
test("un lobby por mesa, idempotente por address; arranca al vencer con ≥4", async () => {
  ...
  const a1 = addr();
  const v1 = await V.joinAleph(0, a1, undefined, T0);
  assert.equal(v1.status, "lobby");
  assert.equal(v1.seats.length, 1);
  assert.equal(v1.closesAt, T0 + V.ALEPH_LOBBY_MS);
  const again = await V.joinAleph(0, a1.toUpperCase().replace("0X", "0x"), undefined, T0 + 1);
  assert.equal(again.roomId, v1.roomId);
  assert.equal(again.seats.length, 1);
  for (let i = 0; i < 3; i++) await V.joinAleph(0, addr(), undefined, T0 + 2);
  assert.deepEqual(
    V.listAlephLobbies(T0 + 3).map((l) => [l.roomId, l.seats]),
    [[v1.roomId, 4]],
  );
  // No vence antes de tiempo.
  assert.equal((await V.getAlephRoom(v1.roomId, a1, T0 + V.ALEPH_LOBBY_MS - 1))!.status, "lobby");
  const started = (await V.getAlephRoom(v1.roomId, a1, T0 + V.ALEPH_LOBBY_MS))!;
  ...
  assert.equal(started.startedAt, T0 + V.ALEPH_LOBBY_MS);
  assert.equal(started.deadline, T0 + V.ALEPH_LOBBY_MS + V.ALEPH_PHASE_MS);
  assert.equal(started.stage!.kind, "share");
  assert.equal(started.you!.status, "alive");
  assert.equal(started.pot, 3200);
  assert.deepEqual(V.listAlephLobbies(T0 + V.ALEPH_LOBBY_MS), []);
  // Sentado en una sala viva: volver a pedir asiento devuelve ESA sala.
  const same = await V.joinAleph(0, a1, undefined, T0 + V.ALEPH_LOBBY_MS + 5);
...
test("con menos de 4 al vencer, el lobby se disuelve y el próximo pedido crea otro", async () => {
  ...
  const a1 = addr();
  const v1 = await V.joinAleph(0, a1, undefined, T0);
  await V.joinAleph(0, addr(), undefined, T0);
  const gone = (await V.getAlephRoom(v1.roomId, a1, T0 + V.ALEPH_LOBBY_MS))!;
  assert.equal(gone.status, "dissolved");
  const v2 = await V.joinAleph(0, a1, undefined, T0 + V.ALEPH_LOBBY_MS + 1);
...
test("con 8 asientos arranca en el acto; el tope de salas vivas corta", async () => {
  V.__resetAlephForTest();
  let last;
  for (let i = 0; i < 8; i++) last = await V.joinAleph(0, addr(), undefined, T0);
  assert.equal(last!.status, "playing");
  assert.equal(last!.seats.length, 8);
  for (let i = 0; i < 8; i++) last = await V.joinAleph(0, addr(), undefined, T0);
  assert.equal(last!.status, "playing");
  await assert.rejects(() => V.joinAleph(0, addr(), undefined, T0), /room limit/);
});

test("validaciones: mesa, address, kill switch y firma", async () => {
  V.__resetAlephForTest();
  await assert.rejects(() => V.joinAleph(1, addr(), undefined, T0), /stake not allowed/);
  await assert.rejects(() => V.joinAleph(0, "0x123", undefined, T0), /invalid address/);
  process.env.ALEPH_ENABLED = "false";
  await assert.rejects(() => V.joinAleph(0, addr(), undefined, T0), /aleph disabled/);
  delete process.env.ALEPH_ENABLED;

  const acc = privateKeyToAccount(generatePrivateKey());
  const me = acc.address.toLowerCase();
  const ts = T0;
  const signature = await acc.signMessage({ message: matchmakeAuthMessage("aleph", 0, me, ts) });
  const ok = await V.joinAleph(0, me, { signature, ts }, T0);
  assert.equal(ok.status, "lobby");
  const other = privateKeyToAccount(generatePrivateKey());
  const bad = await other.signMessage({ message: matchmakeAuthMessage("aleph", 0, me, ts) });
  await assert.rejects(() => V.joinAleph(0, addr(), { signature: bad, ts }, T0), /bad signature/);
  await assert.rejects(
    () => V.joinAleph(0, me, { signature, ts }, T0 + 11 * 60_000),
    /auth expired/,
  );
  await assert.rejects(() => V.joinAleph(0, me, { signature: "0x1234", ts }, T0), /bad signature/);
});
...
test("kill switch: un lobby que vence con el switch apagado se disuelve, no arranca", async () => {
  ...
  for (let i = 0; i < 3; i++) await V.joinAleph(0, addr(), undefined, T0);
  assert.equal(v.status, "lobby");
  process.env.ALEPH_ENABLED = "false";
  try {
    const gone = (await V.getAlephRoom(v.roomId, undefined, T0 + V.ALEPH_LOBBY_MS))!;
...
test("tope de salas terminadas: al pasarse, se purgan las más viejas", async () => {
  ...
  const ids: string[] = [];
  for (let i = 0; i < 3; i++) {
    const at = T0 + i * 2 * V.ALEPH_LOBBY_MS;
    const v = await V.joinAleph(0, addr(), undefined, at);
    ids.push(v.roomId);
    assert.equal(
      (await V.getAlephRoom(v.roomId, undefined, at + V.ALEPH_LOBBY_MS))!.status,
      "dissolved",
    );
  }
  const now = T0 + 6 * V.ALEPH_LOBBY_MS;
  assert.equal(V.ALEPH_MAX_SETTLED_KEPT, 2);
  assert.equal(await V.getAlephRoom(ids[0], undefined, now), null, "la más vieja se purga");
  assert.ok(await V.getAlephRoom(ids[1], undefined, now));
  assert.ok(await V.getAlephRoom(ids[2], undefined, now));
});

test("persistencia: serializar y restaurar conserva el lobby abierto y una sala en juego", async () => {
  V.__resetAlephForTest();
  const a1 = addr();
  const lobby = await V.joinAleph(0, a1, undefined, T0);
  ...
  let playing;
  for (const a of seats8.slice(0, 7)) playing = await V.joinAleph(0, a, undefined, T0);
  assert.equal(playing!.status, "playing");
  const roomId = playing!.roomId;
  assert.equal(roomId, lobby.roomId);
  const lobby2 = await V.joinAleph(0, seats8[7], undefined, T0 + 1);
  assert.equal(lobby2.status, "lobby");

  const raw = V.serializeAleph();
  V.__resetAlephForTest();
  assert.equal(await V.getAlephRoom(roomId, a1, T0 + 2), null);
  V.restoreAlephFrom(raw);
  const back = (await V.getAlephRoom(roomId, a1, T0 + 2))!;
  ...
  const newcomer = await V.joinAleph(0, addr(), undefined, T0 + 3);
  assert.equal(newcomer.roomId, lobby2.roomId);
  assert.equal(newcomer.seats.length, 2);
  assert.deepEqual(
    V.listAlephLobbies(T0 + 4).map((l) => [l.roomId, l.seats]),
    [[lobby2.roomId, 2]],
  );
