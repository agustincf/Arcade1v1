// apps/server/test/aleph-routes.test.ts
// Rutas HTTP de Aleph con REQUIRE_AUTH activado como en producción.
// Correr: node --import tsx --test apps/server/test/aleph-routes.test.ts
...
async function join(acc: PrivateKeyAccount, signer = acc) {
  ...
  const signature = await signer.signMessage({
    message: matchmakeAuthMessage("aleph", 0, low(acc), ts),
  });
  return post("/aleph/join", { stake: 0, address: low(acc), signature, ts });
}
...
async function viewPass(roomId: string, acc: PrivateKeyAccount, signer = acc) {
  const ts = Date.now();
  const signature = await signer.signMessage({
    message: alephViewAuthMessage(roomId, low(acc), ts),
  });
...
async function act(
  ...
  return post(`/aleph/${roomId}/act`, { address: low(acc), stage, phase, action, signature, ts });
}
...
test("join: exige firma propia; lobby visible; vista pública; log cerrado; act en lobby rechazado", async () => {
  ...
  let r = await post("/aleph/join", { stake: 0, address: low(a) });
  ...
  const sig_join = await a.signMessage({
    message: matchmakeAuthMessage("aleph", 0, low(a), ts_join),
  });
  r = await post("/aleph/join", { stake: 0, address: low(a), signature: sig_join });
  assert.equal(r.status, 400);
  assert.match(String(r.body.error), /falta ts/);
  r = await post("/aleph/join", { address: low(a) });
  assert.equal(r.status, 400);
  r = await join(a);
  ...
  r = await get("/aleph/0xnope");
  assert.equal(r.status, 404);
  r = await get(`/aleph/${roomId}/log`);
  assert.equal(r.status, 400);
...
test("act: con 8 asientos arranca; la acción firmada entra; la ajena y la incompleta no", async () => {
  ...
  const sig_act = await accs[1].signMessage({
    message: alephActionAuthMessage(roomId, 0, "decide", actionLine({ type: "keep" }), ts_act),
  });
  r = await post(`/aleph/${roomId}/act`, {
    address: low(accs[1]),
    stage: 0,
    phase: "decide",
    action: { type: "keep" },
  ...
  assert.equal(r.status, 400);
  assert.match(String(r.body.error), /falta ts/);
  r = await post(`/aleph/${roomId}/act`, { address: low(accs[1]), action: { type: "keep" } });
  assert.equal(r.status, 400);
  ...
  r = await get(`/aleph/${roomId}?address=${low(accs[1])}`);
  assert.equal(r.status, 200);
  ...
  r = await get(`/aleph/${roomId}?${await viewPass(roomId, accs[1])}`);
  assert.equal(r.status, 200);
  ...
  r = await get(`/aleph/${roomId}?${await viewPass(roomId, accs[1], accs[2])}`);
  assert.equal(r.status, 200);
